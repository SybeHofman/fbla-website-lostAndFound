...
router.post("/", async (request, response) => {
  ...
  if(!from || !to || !content) {
    return response.status(400).json("From, to, and content are required");
  }
  ...
  database.insert(newData, (error) => {
    if (error) {
      ...
      return response.status(500).json("Error saving message to the database");
    }
    console.log(newData);
  ...
  try{
    ...
    database.find({from: from}, (error, data) => {
      if(error) {
        ...
        return response.status(500).json("Error retrieving messages");
      }

      if(!data) {
        console.log("No from messages found from user:", from);
        return response.status(404).json("No messages found from this user");
      }
